...
public class MainActivity extends BaseActivity implements MainIView, MainAdapter.IDeliveryClick {

    @Inject
    MainIPresenter<MainIView> mPresenter;
    ...
    public void onBackPressed() {
        ...
        Fragment fragment = fragmentManager.findFragmentByTag(DeliveryMapFragment.TAG);
        if (fragment == null) {
            super.onBackPressed();
        } else {
            onFragmentDetached(DeliveryMapFragment.TAG);
        }
    }

    @Override
    public void refreshDeliveries(List<Deliveries> deliveries) {
        lvEmptyData.setVisibility(View.GONE);
        recyclerView.setVisibility(View.VISIBLE);
        recMainAdapter.updateDeliveries(deliveries);
    }
    ...
    @Override
    public void onDeliveryClick(double lat, double lng,String address) {
        lockDrawer();
        getSupportFragmentManager()
                .beginTransaction()
                .disallowAddToBackStack()
                .setCustomAnimations(R.anim.slide_left, R.anim.slide_right)
                .add(R.id.cl_root_view, DeliveryMapFragment.newInstance(lat, lng,address), DeliveryMapFragment.TAG)
                .commit();
    }
